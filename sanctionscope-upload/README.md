...
## Watchlist monitoring (Pro)

Screening answers "is this name listed today". Monitoring answers "tell me when that
changes", which is the part worth a subscription. A Pro customer POSTs a list of
counterparty names to `/api/v1/watchlist`; every night the build matches them against the
rebuilt data and reports what moved.

    POST   /api/v1/watchlist          create or replace a list (<=10 lists, <=5000 names)
    GET    /api/v1/watchlist          your lists, with how many names currently match
    GET    /api/v1/watchlist/<id>     one list, its names and its current matches
    DELETE /api/v1/watchlist/<id>     removes the names and the match history
    GET    /api/v1/alerts             what changed, filterable by since/type/list_id

Alert types: `added` (a watched name now matches a listed party), `removed` (a match was
delisted), `linked` (a match is newly linked to another listed party, which is where the
50% rule bites), `changed` (another authority picked it up), `near` (below the threshold,
opt in with `"near": true`), and `baseline` (the one-off summary on a list's first night).

**The matching runs in the nightly build, not in a Function.** The free Workers plan gives
10 ms of CPU and 50 subrequests per request, and search already spends most of that;
matching thousands of names against 37,000 parties does not fit. `pipeline/monitor.py`
does the work, reads watchlists from the same KV namespace the API keys use, and writes
alerts back. So creating a list returns `"baseline": "queued"`, not results. Point
customers at `/api/v1/screen` when they want an answer immediately.

`pipeline/matching.py` is a line-for-line port of the scoring in
`functions/api/v1/_lib.js`. If the two drift, screening and monitoring disagree and the
customer trusts neither, so `tests/test_matching_parity.py` runs the same name pairs
through both and fails on any difference. Run it after touching either file.

Setup, on top of the Stripe steps above:

1. GitHub repo → Settings → Secrets and variables → Actions → Secrets: `CF_API_TOKEN`,
   `CF_ACCOUNT_ID`, `CF_KV_NAMESPACE_ID` (the AIS collector already uses these), and
   `RESEND_KEY` for email digests. Without them the build prints "monitoring: Cloudflare KV
   not configured, skipped" and carries on.
2. Variables: `ALERT_FROM`, the address digests are sent from, on a domain verified with
   your email provider.
3. `MONITOR_DRY_RUN=1` matches and prints without writing anything, for testing.

KV free tier is 1,000 writes a day and fails hard at the cap. Monitoring costs 2 writes per
customer per night, so roughly 450 customers before Workers Paid ($5/month) is needed.

Before selling this, update `site/privacy.html`: it currently says names sent to the
screening API are not stored, and monitoring stores them by definition. Say what is kept,
for how long, and that DELETE removes it. These are customers' prospective counterparties,
so it is worth a lawyer's eye.
